...
if __name__ == '__main__':
    ...
    def save_image(*, location, array, metadata, height, width):
        ...
        # 转换为PIL图像
        array = array.squeeze(0).squeeze(0)  # 移除批次维度
...
#B_transform =  get_transform(self.opt, transform_params, grayscale=(self.output_nc == 1))
            ...
            image_input = image_input.unsqueeze(1)  # 在第1个位置（从0开始计数）添加维度
            ...
            #print('image_predict',image_predict['fake_B'].shape)
            save_image(location=join(output_organelle_path,basename(input_file_name)), array=image_predict['fake_B'],metadata=metadata,height = height, width = width)
